# 构建测试程序

dotnet new webapi -n helloworld --use-controllers

docker build -t helloworld:serilog_1 .
docker run -d -p 8080:80 -p 8444:444 --name serilogtest helloworld:serilog_1

## 指定 log driver

docker run -d -p 8080:80 -p 8444:444 --name serilogtest_1 --log-driver=fluentd --log-opt fluentd-address=localhost:24224 helloworld:serilog_1

可访问：
https://localhost:8444/WeatherForecast //这个不行，不打 log 的哈

https://localhost:8444/WeatherForecast/api/testlog //这个打 log

是可以在 kibana 中看到 log 结果的
http://localhost:5601/app/discover#/
log 都是分段的

# 现在集成 Serilog

来输出结构化、易查询的 JSON 日志是绝对正确的选择，这将极大提升在 Kibana 中的日志分析体验

dotnet add package Serilog.Sinks.Console
dotnet add package Serilog.AspNetCore

# 🔧 （可选）生产级增强配置

如果你希望对日志级别、输出模板等有更精细的控制，可以在 appsettings.json 中添加 Serilog 配置节。首先，请确保在项目中安装可选包：

```
dotnet add package Serilog.Settings.Configuration
```

docker build -t helloworld:serilog-structured .
docker run -d -p 8080:80 -p 8444:444 --name serilog_structured_test --log-driver=fluentd --log-opt fluentd-address=localhost:24224 helloworld:serilog-structured

# 问题

kibana 中不能很快找到 log 级别，单独成列

这是这直接 postman 查询 ES 得到的记录

```
http://localhost:9200/fluentd-2025.12.21/_search?pretty
 {
                "_index": "fluentd-2025.12.21",
                "_id": "SKO0QJsB9C1BH-C6t-G9",
                "_score": 1.0,
                "_ignored": [
                    "log.keyword"
                ],
                "_source": {
                    "log": "{\"Timestamp\":\"2025-12-21T11:38:58.5478891+00:00\",\"Level\":\"Warning\",\"MessageTemplate\":\"警告级别：这是一个模拟警告\",\"TraceId\":\"082db9623f30859db9cd8f10f7468c2d\",\"SpanId\":\"eb1bfc35880d19ba\",\"Properties\":{\"SourceContext\":\"helloworld.Controllers.WeatherForecastController\",\"ActionId\":\"cb9442d2-3b2d-4923-beea-331df78f6633\",\"ActionName\":\"helloworld.Controllers.WeatherForecastController.TestLog (helloworld)\",\"RequestId\":\"0HNI0FRVV05SP:00000001\",\"RequestPath\":\"/WeatherForecast/api/testlog\",\"ConnectionId\":\"0HNI0FRVV05SP\"}}",
                    "container_id": "cad31e3b529cd886d0817f2abb9eb3371004ac260983b7622d2d7a6f87e27d7e",
                    "container_name": "/serilog_structured_test",
                    "source": "stdout",
                    "hostname": "eab99972c779",
                    "service": "my_windows_app",
                    "@timestamp": "2025-12-21T11:38:58.000000000+00:00"
                }
            },
```

您的 Fluentd 配置没有问题，但不完整。它缺少了一个最关键的步骤：解析来自 Docker 的、包裹在 log 字段中的 JSON 字符串。

因为您的日志虽然以 JSON 格式输出，但 Docker 的 fluentd 驱动将其作为一个整体文本消息发送，导致在 Elasticsearch 中它被存储为一个名为 log 的字符串字段。这就是为什么 Level 等字段在 Kibana 中“隐身”了。

这是因为 flentd 配置文件 决定的，需要研究 fluent.conf 文件的配置

```
<filter **>
  @type parser
  key_name log
  reserve_data true
  remove_key_name_field false
  <parse>
    @type json
  </parse>
</filter>
```

加上这个后 work 了

# docker run --rm --log-driver=fluentd --log-opt fluentd-address=localhost:24224 registry.cn-hangzhou.aliyuncs.com/abelcontainer/alpine:3.19.1 echo "【Docker 驱动测试】这是一条通过容器标准输出生成的日志" 这个方式的 日志，怎么找不到了呢

Fluentd 的处理流程：

这条纯文本被 Docker 驱动捕获，放入 log 字段发送给 Fluentd。

日志进入 \*\* 过滤器链，第一个 record_transformer 为它加上了 hostname 和 service 字段。

接着，它进入了关键的 <filter \*\*> 解析器。这个解析器被配置为 @type json 并 key_name log。

问题发生：解析器尝试将 log 字段的值（“【Docker 驱动测试】...” 这段纯文本）当作 JSON 格式来解析。这显然会失败。

让解析器“宽容”一些（备用方案）

如果环境中有多种日志格式混合，或者你无法预先知道所有标签，可以采用更“宽容”的通用规则，并通过参数防止误伤。

```
<filter **>
  @type parser
  key_name log
  reserve_data true
  suppress_parse_error_log true    # 抑制解析错误的警告日志
  emit_invalid_record_to_error false # 重要：防止无效记录被丢弃
  <parse>
    @type json
  </parse>
</filter>
```

# docker run -d -p 8080:80 -p 8444:444 --name serilog_structured_test --log-driver=fluentd --log-opt fluentd-address=localhost:24224

那就是说运行容器的时候 都要这么指定 logdriver 吗

为了不要总指定 logdriver

```
my_net_app:
    image: helloworld:serilog-structured
    container_name: my_net_app_prod
    ports:
      - "8080:80"
      - "8444:444"
    # 核心配置：在这里统一设置日志驱动
    logging:
      driver: "fluentd"
      options:
        fluentd-address: "localhost:24224"
        # 可以添加更多标签，方便在Fluentd中路由
        tag: "prod.app.{{.Name}}"
    # 确保与Fluentd在同一个网络，才能用localhost访问
    networks:
      - elastic
```

# 此处本地部署测试 log 程序 my_net_app 问题

代码在：D:\Code\1.microservice\7.netcore_microservice_monitor\helloworld
部署在：D:\Code\1.microservice\7.netcore_microservice_monitor\docker-compose.yml

https://localhost:8444/WeatherForecast/api/testlog
生成的 Elastic 的 index 为什么 只有一个 0 号分片，主分片和副本分片

```
http://localhost:9200/_cat/shards/fluentd-2025.12.31?v&h=index,shard,prirep,state,node,store&s=shard,prirep
index              shard prirep state   node      store
fluentd-2025.12.31 0     p      STARTED es-data1 22.4kb
fluentd-2025.12.31 0     r      STARTED es-data2 22.5kb

```

# 方案

问题分析
Fluentd 使用 logstash_format true 时，会自动创建按日期命名的索引（如 fluentd-2025.12.31），并使用 Elasticsearch 的默认模板设置（通常是 1 主分片 1 副本）。

解决方案
有几种方式可以控制 Fluentd 自动创建索引的分片配置：
方案 2：在 Fluentd 配置中指定索引模板

方案 3：直接设置索引设置
如果你不想使用模板，可以在 Fluentd 中直接设置：

```
<match **>
  @type elasticsearch
  host es-master
  port 9200
  logstash_format true
  logstash_prefix fluentd
  number_of_shards 2      # 直接设置分片数
  number_of_replicas 1    # 直接设置副本数
  flush_interval 5s
</match>
```

## 我想选方案 3 这个方式不能工作

docker-compose restart fluentd

Fluentd 的 elasticsearch 输出插件确实不支持直接在配置中设置 number_of_shards 和 number_of_replicas 参数。这是我的错误，我需要更正。

## 选方案 2

1. 首选方案：创建索引模板（必须使用）
   这是唯一可靠的方法。通过 Postman 或 Kibana Dev Tools 执行：

```
PUT /_index_template/fluentd_template
{
  "index_patterns": ["fluentd-*"],
  "template": {
    "settings": {
      "number_of_shards": 2,
      "number_of_replicas": 1,
      "refresh_interval": "30s"
    }
  },
  "priority": 100
}
```

验证模板已创建：

```
GET /_index_template/fluentd_template

```

2. 修改 Fluentd 配置使其支持模板
   修改你的 Fluentd 配置文件:

```
<match **>
  @type elasticsearch
  host es-master
  port 9200
  logstash_format true
  logstash_prefix fluentd

  # 添加模板配置
  template_name "fluentd_template"  # 指定模板名称
  template_overwrite true           # 允许覆盖已存在的模板

  # 可选：如果模板不存在，可以内联定义
  # customize_template '{"index_patterns":["fluentd-*"],"template":{"settings":{"number_of_shards":2,"number_of_replicas":1}}}'

  flush_interval 5s
</match>
```

3. 完整验证步骤

```
# 1. 删除现有索引（让Fluentd重新创建）
DELETE /fluentd-2025.12.31

# 2. 确保模板存在
GET /_index_template/fluentd_template

# 如果不存在，创建它：
PUT /_index_template/fluentd_template
{
  "index_patterns": ["fluentd-*"],
  "template": {
    "settings": {
      "number_of_shards": 3,
      "number_of_replicas": 1
    }
  }
}

# 3. 重启Fluentd
docker-compose restart fluentd

# 4. 生成日志（访问你的应用）
# 访问：https://localhost:8444/WeatherForecast/api/testlog

# 5. 验证新索引的分片配置
GET /fluentd-2025.12.31/_settings

# 6. 查看分片分布
GET /_cat/shards/fluentd-2025.12.31?v
```

## 问题

这个模板是存储在了哪里，是 Elastic search 中的吗？

是的，索引模板是存储在 Elasticsearch 集群中的，具体来说是存储在集群状态（Cluster State）中。

```
# 查看所有模板
GET /_index_template

# 查看特定模板
GET /_index_template/fluentd_template

```

查看分片分布

```
http://localhost:9200/_cat/shards/fluentd-2026.01.03?v&h=index,shard,prirep,state,node,store&s=shard,prirep

index              shard prirep state   node     store
fluentd-2026.01.03 0     p      STARTED es-data1  227b
fluentd-2026.01.03 0     r      STARTED es-data2  227b
fluentd-2026.01.03 1     p      STARTED es-data2  227b
fluentd-2026.01.03 1     r      STARTED es-data1  227b
fluentd-2026.01.03 2     p      STARTED es-data1 6.8kb
fluentd-2026.01.03 2     r      STARTED es-data2 6.9kb

```

满足预期
